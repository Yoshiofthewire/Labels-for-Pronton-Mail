...
export function ConfigPage() {
  ...
  const [testPrompt, setTestPrompt] = useState("Return only the label Questionable");
  ...
  return (
    <section className="panel">
      <h2>Configuration</h2>
      <p>Configure Lumo connection and app scan limits.</p>

      <label>
        <div>Lumo Base URL</div>
        <input
          value={cfg.lumo?.baseUrl ?? ""}
          onChange={(e) => setCfg({ ...cfg, lumo: { ...cfg.lumo, baseUrl: e.target.value } })}
        />
      </label>

      <label>
        <div>Lumo API Key</div>
        <input
          value={cfg.lumo?.apiKey ?? ""}
          onChange={(e) => setCfg({ ...cfg, lumo: { ...cfg.lumo, apiKey: e.target.value } })}
        />
      </label>

      <label>
        <div>Lumo Classify Path</div>
        <input
          value={cfg.lumo?.classifyPath ?? "/"}
          onChange={(e) => setCfg({ ...cfg, lumo: { ...cfg.lumo, classifyPath: e.target.value } })}
        />
      </label>
      ...
      <p>Edit TUNING.md content that is sent after GARDRAIL.md and before the email content.</p>

      <h4>Proton Labels</h4>
      <button type="button" onClick={syncLabels}>
        Sync Labels from Proton
      </button>
      ...
      {orderedLabels.map((label, idx) => (
        <div key={label} style={{ border: "1px solid var(--line)", borderRadius: 6, padding: 8, marginBottom: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
            <strong>{label}</strong>
            <button type="button" onClick={() => moveLabel(idx, -1)} disabled={idx === 0}>
              Up
            </button>
            <button type="button" onClick={() => moveLabel(idx, 1)} disabled={idx === orderedLabels.length - 1}>
              Down
            </button>
          </div>
          <textarea
      ...
      <button type="button" onClick={buildTuningFromLabels}>
        Build TUNING from Labels
      </button>
      <button type="button" onClick={resetTuningTemplate} style={{ marginLeft: 8 }}>
        Reset TUNING Template
      </button>
      ...
      <button type="button" onClick={saveTuning}>
        Save TUNING.md
      </button>
